...
def line_message(message, access_token=False):
...
def mugen(access_token, base_url: str, suffix: List[int]) -> None:
    while True:
        found = False
        for hour in suffix:
            res = requests.get(base_url + str(hour))
            if "～" in res.text:
                message = str(hour) + "時台の空きを発見！"
                print(message)
                found = True
                wav_obj = simpleaudio.WaveObject.from_wave_file("発見の音.wav")
                play_obj = wav_obj.play()
                play_obj.wait_done()
                line_message(message, access_token)
        else:
            if not found:
...
def do_mugen(access_token, store_id, hours_to_search):
    ...
    try:
        mugen(access_token, base_url, hours_to_search)
    except:
        print("エラーが発生したので再起動しました。")
        print("日付変更以外でこれが表示された場合は、設定方法が間違っている可能性があります。")
        do_mugen(access_token, store_id, hours_to_search)
